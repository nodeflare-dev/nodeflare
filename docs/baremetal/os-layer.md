...
# Unix Socketベースディレクトリ（tmpfs）
# テナント専用サブディレクトリは Builder が作成
tmpfs /var/run/mcp tmpfs size=512M,mode=0755,nodev,nosuid,noexec 0 0
```
...
| `mode=0755` | ベースディレクトリは読み取り可能 | サブディレクトリはBuilderがmode 770で作成 |

**ソケットディレクトリ構成**:
```
/var/run/mcp/           # tmpfs, mode=0755
├── server-abc123/      # mode=0770, テナントA専用
│   └── default.sock
├── server-def456/      # mode=0770, テナントB専用
│   └── default.sock
```
